  ...
  def self.import(import_params)
    CSV.foreach(import_params[:file].path, headers: true, col_sep: ";") do |row|
      ...
      if location
        ...
        mangrove_datum_hash.gain_m2 = comma_conversion(row["gain_m2"])
        mangrove_datum_hash.loss_m2 = comma_conversion(row["loss_m2"])
        mangrove_datum_hash.length_m = comma_conversion(row["length_m"])
        mangrove_datum_hash.area_m2 = comma_conversion(row["area_m2"])
        mangrove_datum_hash.hmax_m = comma_conversion(row["hmax_mangrove_m"])
        mangrove_datum_hash.agb_mgha_1 = comma_conversion(row["agb_mangrove_mgha-1"])
        mangrove_datum_hash.hba_m = comma_conversion(row["hba_mangrove_m"])
    ...
    mangrove_datum["features"].each do |mangrove_data|
      ...
      if location
        ...
        years.each do |year|
          ...
          if !row["area_mangrove_gain_m2"].nil? && !row["area_mangrove_loss_m2"].nil?
